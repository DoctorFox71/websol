(() => {
 ...
 /*! smooth-scroll v16.1.3 | (c) 2020 Chris Ferdinandi | MIT License | http://github.com/cferdinandi/smooth-scroll */            window.Element && !Element.prototype.closest && (Element.prototype.closest = function(e) {
    ...
    (() => {
        ...
        let gotoblock_gotoBlock = (targetBlock, noHeader = false, speed = 2500, offsetTop = 0) => {
        ...
        pageNavigation();
        var app_elements = [ {
            titleSelector: ".dofamin-spoiler-3__title",
            targetElement: document.getElementById("dof-pentagon")
        }, {
            titleSelector: ".dofamin-spoiler-1__title",
            targetElement: document.getElementById("circle__top-left")
        }, {
            titleSelector: ".dofamin-spoiler-2__title",
            targetElement: document.getElementById("circle__top-right")
        }, {
            titleSelector: ".dofamin-spoiler-4__title",
            targetElement: document.getElementById("circle__bot-1")
        }, {
            titleSelector: ".dofamin-spoiler-5__title",
            targetElement: document.getElementById("circle__bot-2")
        }, {
            titleSelector: ".dofamin-spoiler-6__title",
            targetElement: document.getElementById("circle__bot-3")
        } ];
        function changeColors(targetElement, isHovered, isClicked) {
            var strokeColor = isClicked ? "#F01248" : isHovered ? "#F01248" : "#F5D7D7";
            var fillColor = isHovered ? "#F01248" : "transparent";
            targetElement.setAttribute("stroke", strokeColor);
            targetElement.setAttribute("fill", fillColor);
        }
        function initializeEvents() {
            app_elements.forEach((function(element) {
                var title = document.querySelector(element.titleSelector);
                var isHovered = false;
                var isClicked = false;
                var activeElement = element.targetElement;
                function hoverHandler() {
                    isHovered = true;
                    changeColors(activeElement, isHovered, isClicked);
                }
                ...
                title.addEventListener("mouseenter", hoverHandler);
                title.addEventListener("mouseleave", unhoverHandler);
                title.addEventListener("click", clickHandler);
            }));
        }
        if (window.innerWidth > 1e3) initializeEvents();
        ...
        function submitForm() {
            ...
            xhr.onload = function() {
                ...
                } else console.error("Произошла ошибка при отправке формы");
            };
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            function scrollToBlock(event) {
                if (window.innerWidth <= 999) if (canScroll) {
                    event.preventDefault();
                    const targetClass = event.target.getAttribute("data-scroll");
                    const block = document.querySelector(`.${targetClass}`);
                    if (block) {
